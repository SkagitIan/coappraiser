...
class BillingTests(TestCase):
    ...
    def test_public_pricing_shows_configured_plan_catalog(self):
        response = self.client.get(reverse("pricing"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Starter")
        self.assertContains(response, PLANS["pro"]["price"])

    def test_checkout_requires_login(self):
        response = self.client.get(reverse("billing:start_checkout") + "?plan=pro")
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('billing:start_checkout')}%3Fplan%3Dpro")

    def test_mock_checkout_creates_active_subscription(self):
        self.client.login(username="subscriber", password="pass12345")
        response = self.client.post(reverse("billing:checkout"), {"plan": "pro"})
    ...
    def test_signup_to_plan_to_mock_checkout_flow(self):
        ...
        self.assertRedirects(signup, reverse("assignments:dashboard"))
        pricing = self.client.get(reverse("pricing"))
        self.assertContains(pricing, reverse("billing:start_checkout") + "?plan=pro")
        confirm = self.client.get(reverse("billing:start_checkout") + "?plan=pro")
        self.assertEqual(confirm.status_code, 200)
        checkout = self.client.post(reverse("billing:checkout"), {"plan": "pro"})
    ...
    def test_stripe_checkout_uses_server_side_price(self, stripe_client_mock, get_plan_mock):
        get_plan_mock.return_value = {"name": "Pro", "price": "$99/month", "price_id": "price_server_pro"}
        stripe = Mock()
        ...
        self.assertEqual(kwargs["line_items"][0]["price"], "price_server_pro")
    ...
    def test_mock_webhook_is_idempotent_and_syncs_subscription(self):
        event = {"id": "evt_test_1", "type": "customer.subscription.updated", "data": {"object": {"id": "sub_test_1", "customer": "cus_test_1", "status": "active", "metadata": {"user_id": str(self.user.pk), "plan": "pro"}, "items": {"data": [{"price": {"id": "price_test"}}]}, "current_period_start": 1700000000, "current_period_end": 1702600000, "cancel_at_period_end": False}}}
    ...
    def test_past_due_user_keeps_billing_access_but_not_paid_workflows(self):
        Subscription.objects.create(user=self.user, status="past_due", plan="pro")
        self.client.login(username="subscriber", password="pass12345")
